...
event.preventDefault(); monthlyIncome = incomeInput.value; monthlyBudget.innerText = 
...
function addExpense(event) {
    ...
    let expense = {       expenseName:
    nameInput.value,      expenseAmount:
    amountInput.value
};
...
function updateExpenseTotal() {
    expenseTotal = 0;
    for (let i = 0; i < expenses.length; i++) {   expenseTotal +=
    expenses[i]; 
} totalExpenses.innerText = "$" + expenseTotal; updateBalance();
}
